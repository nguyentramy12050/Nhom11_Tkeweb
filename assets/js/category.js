...
// Hàm phụ trợ: Ép kiểu giá từ chuỗi (VD: "185.000") thành số (185000) để máy tính hiểu
...
function filterByPrice(tier) {
    // Cập nhật biến mức giá và chạy bộ lọc tổng
    currentPriceTier = tier;
    applyAllFilters();
}

// Bộ lọc tổng (Kết hợp cả 2 điều kiện)
function applyAllFilters() {
    let tempBooks = [...booksData];

    // 1. Lọc theo Thể loại trước
    if (currentCategory !== 'all') {
        tempBooks = tempBooks.filter(book => book.category === currentCategory);
    }

    // 2. Lọc tiếp theo Mức giá
...
// ==========================================
// 4. HÀM RENDER SÁCH VÀ PHÂN TRANG
// ==========================================
        ...
        const bookCard = `
            <div class="book-card">
                <div class="book-img-wrapper">
                    <img src="assets/images/books/${book.image}" alt="${book.name}" onerror="this.src='assets/images/logo.png'">
                    ${badgeHTML}
